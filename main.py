...
def parse_document_from_url(file_url: str, ext: str) -> str:
    ...
    try:
        ...
        if ext == ".pdf":
            text = ""
            # Wrap bytes in a BytesIO stream for PdfReader
...
async def run_ai_ingestion_pipeline(
    ...
    try:
        ...
        metadatas = [
            {"source": original_filename, "doc_id": doc_id, "cloudinary_url": file_url}
            for _ in chunks
        ]
...
async def upload_document(
    ...
    return {
        "message": "File successfully uploaded to Cloudinary and accepted.",
